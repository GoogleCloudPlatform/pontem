...
class MySQL(object):
  ...
  def get_views(self):
    ...
    views = []
    cursor = self._connection.cursor()
    cursor.execute(mysql_constants.MY_SQL_LIST_VIEWS_QUERY.format(self._database))
  ...
  def dump_sql(self, bucket_url):
    ...
    created, mysqldump is not checked for errors.  The caller should validate a
    dump file was created in the Cloud Storage Bucket.
    ...
    ms = subprocess.Popen(command_args, stdout=subprocess.PIPE)
    gz = subprocess.Popen(('gzip'), stdin=ms.stdout, stdout=subprocess.PIPE)
    gs = subprocess.Popen(('gsutil', 'cp', '-', bucket_url), stdin=gz.stdout)
    _, error = gs.communicate()
    if error:
