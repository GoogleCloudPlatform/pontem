...
from google.pontem.sql.replicator.install.util import constants
from google.pontem.sql.replicator.install.util import utils
from google.pontem.sql.replicator.install.util import validation_errors
...
def config_set_project(project_id):
    """Sets the gcloud config to specified project id
...
def enable_apis(dry_run=False):
    """Enable necessary APIs for Pontem.
...
def grant_replicator_svc_acct_roles(project_id,
                                    gcp_service_account,
    ...
    roles = {
        'replicator_project': constants.PROJECT_IAM_ROLES_CLIENT
    }

    # replicator doesn't have target id and gsuite account.
    target_id = ''
...
def _grant_bucket_roles(gcp_service_account,
    ...
    if failed_commands:
        file_name = 'grant_replicator_roles.sh'
...
def _grant_roles(roles_map, target_id, project_id,
    ...
    for (resource_type, roles) in roles_map.iteritems():
        ...
        if resource_type == 'replicator_project':
    ...
    while not target_id:
        ...
        if not orgs:
            print('\nYou don\'t have access to any organizations. '
                  'Choose another option to enable Pontem Cloud SQL Replicator '
                  'access.')
            return None
        ...
        choice = input('Enter the organization id where '
                       'you want Pontem Cloud SQL Replicator to run: ').strip()
        try:
...
def choose_project():
    ...
    while not target_id:
        target_id = input(
            ...
            'where you want Pontem Cloud SQL Replicator to run: ').strip()
    return target_id
...
def get_vm_instance_info(instance_name, try_match=False):
    """Get Cloud SQL VM instance ip and zone information if exists, exit if not.
    ...
    Returns:
        str: IP address of the Cloud SQL VM instance.
        str: Zone of the Cloud SQL VM instance, default to 'us-central1-c'.
        str: Name of the Cloud SQL VM instance.
    """
    ...
    try:
        ...
        print('Error retrieving vm instance ip address, '
...
def check_vm_init_status(vm_name, zone):
    ...
    if err:
        ...
        print(err)
        raise validation_errors.SSHError
